...
class leaky_firing_RNN(nn.Module):
    '''
    This is the Leaky-Firing Rate RNN class solved by Forward Euler Approx. (Explicit)
    It's dynamical equation is
    r(t) = (1 - alpha) * r(t-1) + alpha * tanh(W_in * x(t-1) + W_rec * r(t-1) + b + e)
    '''
    ...
    def forward_full(self, x, h):
        ...
        for t in range(x.size(1)):
            ...
            h_rec = torch.matmul(h, self.W_rec) + self.bias_rec
            h_in = torch.matmul(x[:, t], self.W_in) + self.bias_in
            if t>0:
                z_old = z.clone();
            h = h_from_prev + self.alpha * torch.tanh(h_rec + h_in + noise)
            z = h_rec + h_in
    ...
    def forward_lr(self, x, h):
        ...
        for t in range(x.size(1)):
            h_from_prev = (1 - self.alpha) * h
            h_rec = torch.matmul(torch.matmul(h, self.S_l), self.S_r) + self.bias_rec
            h_in = torch.matmul(x[:, t], self.W_in) + self.bias_in
